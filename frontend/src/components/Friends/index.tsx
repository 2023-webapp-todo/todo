...
export default function Friends({ users}:FriendsProps) {
  ...
  const loginUserNickname = JSON.parse(localStorage.getItem("user") || "").nickname;
  ...
  return (
    <ul className={styles.container}>
      <li onClick={() => onClickUser(loginUserId, loginUserNickname)}><p>{loginUserNickname}</p></li>
      {users.map(user => <li key={user.user_id} onClick={()=> onClickUser(user.user_id, user.nickname)}><p>{user.nickname}</p></li>)}
    </ul>
  );
}
